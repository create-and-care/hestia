module Ui
  class NativeSelectComponent < ApplicationComponent
    def initialize(name: nil, options: [], selected: nil, disabled: false, invalid: false, html_options: {})
      @name = name
      @options = options
      @selected = selected
      @disabled = disabled
    ...
    def call
      content_tag :select, name: @name, disabled: @disabled, "aria-invalid": @invalid, **@html_options.except(:class),
        class: cn(
          "flex h-9 w-full appearance-none rounded-md border bg-container px-3 text-sm text-primary",
