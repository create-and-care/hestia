...
module Reminders
  class DeliverDueTest < ActiveSupport::TestCase
    ...
    test "delivers an event reminder once its delay before the occurrence is reached" do
      event = calendar_events(:alpha_meeting) # starts_at: 2.days.from_now
      reminder = EventReminder.create!(calendar_event: event, user: users(:one), minutes_before: 2.days.in_minutes.to_i + 60)
